...
func init() {
	android.RegisterModuleType("stealth_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = stealthExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = stealthExpandVariables(ctx, dep_file)
	...
	cmd := stealthExpandVariables(ctx, String(g.properties.Cmd))
